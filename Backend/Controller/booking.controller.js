...
export const createBookingIVR = async (req, res) => {
    ...
    try {
        ...
        if (!callerPhone || !product || !weightDigits) {
            return res.send(`
        <Response>
          <Say>Sorry, we could not read your input. Please call again.</Say>
        </Response>
      `);
        }
    ...
    `);
        ...
        return res.send(`
      <Response>
        <Say>Sorry, something went wrong on our end. Please try again later.</Say>
      </Response>
    `);
    }
};
