...
const GovtRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${CLIENT_URL || ""}/api/auth/govt_register`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: form.officerName.trim(),
          ...
          {submitted ? (
            ...
            >
              ...
              <motion.button
                ...
                className="w-full py-3 rounded-xl bg-gradient-to-r from-green-500 to-blue-500 text-white font-semibold hover:from-green-600 hover:to-blue-600 transition disabled:opacity-60"
              >
                {isSubmitting ? "Registering..." : "Register Government Account"}
              </motion.button>
            </motion.form>
          )}
        </AnimatePresence>
